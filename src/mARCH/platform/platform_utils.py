...
class ConsoleInfo:
    ...
    def supports_colors(self) -> bool:
        ...
        if "TERM" in os.environ:
            ...
            return bool(term != "dumb" and term != "")
        # Default to true on Unix-like systems
        return bool(PlatformInfo().is_unix_like)
...
class PathUtils:
    ...
    def get_app_data_dir() -> str:
        ...
        if platform_info.is_windows:
            # Windows: %APPDATA%\march or %LOCALAPPDATA%\march
            appdata = os.environ.get("APPDATA") or os.environ.get("LOCALAPPDATA")
            if appdata:
                return str(Path(appdata) / "march")
        elif platform_info.is_macos:
            # macOS: ~/Library/Application Support/march
            return str(Path.home() / "Library" / "Application Support" / "march")

        # Linux and fallback: ~/.march
        return str(Path.home() / ".march")
    ...
    def get_cache_dir() -> str:
        ...
        if platform_info.is_windows:
            ...
            if cache:
                return str(Path(cache) / "march-cache")
        elif platform_info.is_macos:
            return str(Path.home() / "Library" / "Caches" / "march")

        # Linux and fallback
        cache = os.environ.get("XDG_CACHE_HOME") or str(Path.home() / ".cache")
        return str(Path(cache) / "march")
    ...
    def get_config_dir() -> str:
        ...
        if platform_info.is_windows:
            ...
            if config:
                return str(Path(config) / "march")
        elif platform_info.is_macos:
            return str(Path.home() / "Library" / "Preferences" / "march")
